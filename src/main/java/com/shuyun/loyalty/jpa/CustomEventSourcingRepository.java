...
public class CustomEventSourcingRepository<T> extends EventSourcingRepository<T> {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(CustomEventSourcingRepository.class);
    ...
    protected EventSourcedAggregate<T> doLoadWithLock(String aggregateIdentifier,
        ...
        SnapshotTrigger trigger =
                snapshotTriggerDefinition.prepareTrigger(aggregateFactory.getAggregateType());
    ...
    protected LockAwareAggregate<T, EventSourcedAggregate<T>> doLoad(String aggregateIdentifier,
            Instant timestamp) {
        Lock lock = lockFactory.obtainLock(aggregateIdentifier);
        try {
            final EventSourcedAggregate<T> aggregate =
                    doLoadWithLock(aggregateIdentifier, timestamp);
            CurrentUnitOfWork.get().onCleanup(u -> lock.release());
            return new LockAwareAggregate<>(aggregate, lock);
        } catch (Exception ex) {
            LOGGER.debug("Exception occurred while trying to load an aggregate. Releasing lock.",
                    ex);
            lock.release();
            throw ex;
        }
    ...
    public LockAwareAggregate<T, EventSourcedAggregate<T>> load(String aggregateIdentifier,
            Instant timestamp) {
        ...
        Map<String, LockAwareAggregate<T, EventSourcedAggregate<T>>> aggregates =
                managedAggregates(uow);
        LockAwareAggregate<T, EventSourcedAggregate<T>> aggregate = aggregates
                .computeIfAbsent(aggregateIdentifier, s -> doLoad(aggregateIdentifier, timestamp));
    ...
    public static class Builder<T> extends EventSourcingRepository.Builder<T> {
         ...
         *
         * @param aggregateType the {@code aggregateType} specifying the type of aggregate this
        ...
        @Override
        public Builder<T> parameterResolverFactory(
        ...
        /**
         * {@inheritDoc} If this Builder is used to instantiate a
         * {@link CachingEventSourcingRepository}, do note that an optimistic locking strategy is
         * not compatible with a caching approach.
         */
        @Override
        ...
        /**
         * Sets the {@link EventStore} that holds the event com.craftsman.eventsourcing.stream this
         * repository needs to event source an Aggregate.
         *
         * @param eventStore an {@link EventStore} that holds the event
         *        com.craftsman.eventsourcing.stream this repository needs to event source an
         *        Aggregate
        ...
        /**
         * Sets the {@link SnapshotTriggerDefinition} specifying when to trigger a snapshot for an
         * Aggregate contained in this repository.
         *
         * @param snapshotTriggerDefinition a {@link SnapshotTriggerDefinition} specifying when to
         *        trigger a snapshot for an Aggregate contained in this repository
         * @return the current Builder instance, for fluent interfacing
         */
        public Builder<T> snapshotTriggerDefinition(
         ...
         *
         * @param aggregateFactory the {@link AggregateFactory} used to create new Aggregate
         *        instances
         * @return the current Builder instance, for fluent interfacing
         */
        ...
        /**
         * Sets the {@link RepositoryProvider} which services repositories for specific aggregate
         * types.
         *
         * @param repositoryProvider a {@link RepositoryProvider} servicing repositories for
         *        specific aggregate types
         * @return the current Builder instance, for fluent interfacing
         */
         ...
         * as specified through this Builder. Will return a CachingEventSourcingRepository if
         * {@link #cache(Cache)} has been set. Otherwise builds a regular EventSourcingRepository
         *
         * @param <R> a generic extending {@link EventSourcingRepository}, so allowing both an
         *        EventSourcingRepository and {@link CachingEventSourcingRepository} return type
         * @return a {@link EventSourcingRepository} or {@link CachingEventSourcingRepository} (if
        ...
        public <R extends EventSourcingRepository<T>> R build() {
            ...
            if (cache != null) {
                LOGGER.warn("do not support caching repository!!");
            }
         ...
         * {@link EventSourcingRepository} will instantiate based on an event
         * com.craftsman.eventsourcing.stream.
         *
        ...
        protected void validate() {
            super.validate();
            assertNonNull(eventStore,
                    "The EventStore is a hard requirement and should be provided");
            if (aggregateFactory == null) {
                assertNonNull(aggregateType,
                        "No AggregateFactory is set, whilst either it or the aggregateType is a hard requirement");
                return;
            }
            assertNonNull(aggregateFactory,
